...
export default function TeacherDocuments() {
  ...
  const fetchOptions = async () => {
    try {
      // Teachers might have limited scopes for classes/modules depending on the system
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.append('is_public', '0'); // Teachers usually target specific classes
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Mes Documents"
        description={`Partagez des supports de cours, TPs et examens avec vos classes.`}
        actions={<Button leftIcon={<Plus className="h-4 w-4" />} onClick={() => setIsModalOpen(true)}>Uploader</Button>}
      />
